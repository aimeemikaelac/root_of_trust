...
from Cryptodome.Random import getrandbits
...
def render_bif(bif_file, psk_file, ssk_file):
    with open(bif_file) as bif:
        ...
        bif_out_file = os.path.basename(args.operational_os_bif_template).split(".")[0]
...
def generate_boot_image(bootgen_path, bif_path, psk_hash_path, binary_path):
    call_str = "{} -image {} -generate_hashes -efuseppkbits {} -arch zynqmp -log trace -w -o i {}".formar(bootgen_path, bif_path, psk_hash_path, binary_path)
    call_tokens = shlex.split(call_str)
    return subprocess.call(call_tokens)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--secure_storage_file",
                        ...
                        "the extension \".iv\"",
                        default=DEFAULT_SECURE_STORAGE_FILE)
    ...
    parser.add_argument("--efuse_aes_key",
    ...
    if os.path.exists(SECURE_STORAGE_FILE) and not args.reprovision:
    ...
    efuse_aes_key = getrandbits(256)

    #write RSA keys in PEM format
    authentication_key_pem = authentication_key.export()
    ssk_pem = secondary_private_key.export()
    ...
    render_bif(args.operational_os_bif_template,
               authentication_key_storage_file,
               ssk_storage_file)

    render_bif(args.recovery_os_bif_template,
               authentication_key_storage_file,
               ssk_storage_file)

    #generate boot images
    generate_boot_image(args.bootgen_path,
                        args.operational_os_bif_template,
                        args.psk_efuse_hash,
                        args.sdcard_path + "/" + args.operational_os_binary)

    generate_boot_image(args.bootgen_path,
                        args.recovery_os_bif_template,
                        args.psk_efuse_hash,
                        args.sdcard_path +"/" + args.recovery_os_binary)

    #write AES eFuse key to file
    with open(args.aes_efuse_key, 'w') as aes_file:
    ...
    secure_storage_iv = getrandbits(128)
    gcm_cipher = AES.new(efuse_aes_key, MODE_GCM, iv=secure_storage_iv)
