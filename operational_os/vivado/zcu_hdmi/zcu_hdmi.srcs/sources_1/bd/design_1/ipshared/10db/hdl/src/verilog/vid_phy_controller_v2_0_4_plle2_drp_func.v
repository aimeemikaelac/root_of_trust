...
function [`FIXED_WIDTH:1] round_frac_pll
   (
...
`ifdef DEBUG
      $display("round_frac_pll - decimal: %h, precision: %h", decimal, precision);
`endif
      // If the fractional precision bit is high then round up
      if( decimal[(`FRAC_PRECISION-precision)] == 1'b1) begin
         round_frac_pll = decimal + (1'b1 << (`FRAC_PRECISION-precision));
      end else begin
         round_frac_pll = decimal;
      end
`ifdef DEBUG
      $display("round_frac_pll: %h", round_frac_pll);
`endif
   end
endfunction
...
`endif
      ...
      if(divide == 7'h01) begin
         ...
         temp = round_frac_pll(duty_cycle_fix*divide, 1);

         // comes from above round_frac_pll
         high_time   = temp[`FRAC_PRECISION+7:`FRAC_PRECISION+1]; 
         // If the duty cycle * divide rounded is .5 or greater then this bit
         //    is set.
         w_edge      = temp[`FRAC_PRECISION]; // comes from round_frac_pll
...
`endif  
	 ...
	 temp  =  round_frac_pll(phase_in_cycles, 3);
